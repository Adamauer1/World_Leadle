...
async function loadLeaders() {
  const inbound = await fetch("data/leaders.json").then(function (res) {
    return res.json();
  });
  //console.log(inbound)
...
function initBoard() {
  if (checkIfMobile()) {
    alert(
      "For a better experiance play on a computer and not on a mobile device! This website is not yet setup yet for a smaller screen size."
    );
  }
